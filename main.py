...
class LinePath:
    ...
    def draw(self, history, color, shader_idx, model_matrix, active_z_scale):
        ...
        for off in offsets:
            m_off = np.matmul(model_matrix, translate(off, off, 0.0))
...
def main():
    ...
    window = glfw.create_window(win_w, win_h, "Computer Graphics Assignment 1 - VIP Build", None, None)
    ...
    agent_sphere = SphereLatLong(radius=1.0) 
    ...
    def set_race_track(loss_name, keep_pos=False):
        ...
        micro_offsets = [-0.06, -0.03, 0.0, 0.03, 0.06]
    ...
    def on_key(win, key, scancode, action, mods):
        ...
        if action == glfw.PRESS:
            if key == glfw.KEY_1:
                gui.lights[0] = not gui.lights[0]
            elif key == glfw.KEY_2:
                gui.lights[1] = not gui.lights[1]
            elif key == glfw.KEY_3:
                gui.lights[2] = not gui.lights[2]
    ...
    def on_mouse_click(win, button, action, mods):
        ...
        if button == glfw.MOUSE_BUTTON_LEFT and action == glfw.PRESS and not gui.is_ai_mode:
            if mods & glfw.MOD_SHIFT or mods & glfw.MOD_CONTROL:
                return
    ...
    while not glfw.window_should_close(window):
        ...
        if getattr(gui, 'duplicate_obj_requested', False):
            if len(scene_objects) > 0:
                ...
                new_obj.rot_x = old_obj.rot_x
                new_obj.rot_y = old_obj.rot_y
                
                new_obj.pos_x = old_obj.pos_x + 2.0
                new_obj.pos_y = old_obj.pos_y
                new_obj.pos_z = old_obj.pos_z
        ...
        if gui.reset_requested:
            if gui.is_ai_mode:
                set_race_track(gui.loss_funcs[gui.selected_loss_idx], keep_pos=False)
            gui.reset_requested = False
            gui.sim_playing = False

        if gui.reposition_requested:
            if gui.is_ai_mode:
                set_race_track(gui.loss_funcs[gui.selected_loss_idx], keep_pos=True)
        ...
        if gui.add_shape_requested:
            ...
            if idx == 1:
                new_shape = RegularPolygon(sides=3)
            elif idx == 2:
                new_shape = Rectangle()
            elif idx == 3:
                new_shape = RegularPolygon(sides=5)
            elif idx == 4:
                new_shape = RegularPolygon(sides=6)
            elif idx == 5:
                new_shape = RegularPolygon(sides=36)
            elif idx == 6:
                new_shape = Ellipse()
            elif idx == 7:
                new_shape = Trapezoid()
                ...
                new_shape = TruncatedCone()
            elif idx == 13:
                new_shape = Torus()
            elif idx == 14:
                new_shape = SphereLatLong()
            elif idx == 15:
                new_shape = SphereSubdivision() 
            elif idx == 16:
                new_shape = SphereCube()       
            elif idx == 17:
                new_shape = Cube()               
            elif idx == 18:
                new_shape = Tetrahedron()        
            elif idx == 19:
                new_shape = Cylinder(segments=6)
            elif idx == 20:
                ...
                if obj_path.lower().endswith(".ply"):
                    new_shape = PlyModel(filepath=os.path.join("assets", "models", obj_path))
                    shape_name = "PLY Model"
                else:
                    new_shape = ObjModel(filepath=os.path.join("assets", "models", obj_path))
                    shape_name = "OBJ Model"
                    
            if new_shape is not None:
                obj = SceneObject(new_shape, f"#{len(scene_objects)+1} {shape_name}")
                obj.pos_x = gui.spawn_pos[0]
                obj.pos_y = gui.spawn_pos[1]
                obj.pos_z = gui.spawn_pos[2]
        ...
        if gui.is_ai_mode and getattr(gui, 'sim_playing', False) and current_epoch < gui.max_epochs and ai_shape is not None:
            ...
            if current_time - last_step_time >= time_per_step:
                ...
                for i, opt in enumerate(optims):
                    if gui.opt_active[i]:
                        if not hasattr(opt, 'last_pos'):
                            opt.last_pos = np.array([opt.x, opt.y])
                        if not hasattr(opt, 'rot_mat'):
                            opt.rot_mat = np.identity(4)
