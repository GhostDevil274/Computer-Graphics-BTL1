...
void main() {
    ...
    // PHẦN 1: TÍNH TOÁN MÀU GỐC (BASE COLOR) TÙY THEO MODE
    if (render_mode == 0 || render_mode == 3) {
        // Mode 0 & 3: Xài màu sơn tĩnh điện
        baseCol = flat_color; 
    } 
    else if (render_mode == 1 || render_mode == 2) {
        // Mode 1 & 2: Xài màu tự do của đỉnh (Cầu vồng)
        baseCol = vertexColor; 
    } 
    else if (render_mode == 4) {
        // Mode 4: Nạp ảnh Texture thuần túy
        baseCol = texture(tex_diffuse, TexCoord).rgb; 
    }
    else if (render_mode == 5) {
        // Mode 5 (MỚI): Mix ảnh Texture VỚI màu sơn AI (Dành riêng cho trái bi lăn)
        vec4 texColor = texture(tex_diffuse, TexCoord);
        baseCol = texColor.rgb * flat_color;
    }

    vec3 final_color = ambientStrength * baseCol;

    // PHẦN 2: XỬ LÝ ÁNH SÁNG
    if (render_mode == 0 || render_mode == 1 || render_mode == 5) {
        // UNLIT: Trả thẳng màu gốc không đánh đèn (Bi AI giờ sẽ SÁNG RỰC RỠ)
        FragColor = vec4(baseCol, 1.0); 
    } 
    else {
        // LIT: Bật đèn chiếu sáng (Dùng cho Mode 2, 3, 4)
